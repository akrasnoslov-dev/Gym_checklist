...
The exact prefix `02 — Current App / ` applies to every full frame name below. These 64 source-audited entries are the construction inventory; no Pencil frames have been created yet. A grouped evidence frame can contain individually named alternative-state layers; alternatives must not be portrayed as simultaneous application controls.

## Exact ordered Pencil frame names

Every name below is literal, including the prefix. The number matches the table row immediately below it.
...
No `.pen` file is created or modified in this Phase 2 documentation step. The Pencil connector currently targets the Phase 1 canvas instead of a distinct current-app section, so the 64 frames above are intentionally deferred to a separate Pencil step. No contact sheet, iOS screenshot, or Pencil Desktop-open verification is claimed.
