...
| ACC-02 Set editor | New/edited sets have explicit weighted, reps-only, or timed types; irrelevant fields are hidden. Legacy mixed records retain every stored value with a compatibility marker until a user deliberately chooses a type. | Focused simulator and device interaction review. |
...
| ACC-06 Copy | Copy uses source/destination cards with visible disabled-date feedback and a full-width primary action. | Candidate/full CI, then device flow review. |
