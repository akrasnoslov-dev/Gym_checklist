...
| ACC-04 Program hierarchy | Native destructive confirmation covers workout, exercise (including empty exercise), and set deletion; semantic status treatment is implemented. Final device hierarchy review is not yet complete. | Next candidate/full CI, then device hierarchy review. |
| ACC-05 Today | The one-tap flow remains unchanged; lime/mint stays semantic with contrast-safe foreground and primary-control colors, and the completion overlay remains modal. | Next candidate/full CI, then device visual/accessibility review. |
