...
Earlier implementation checkpoints remain pending authoritative macOS/live verification. Public-repository macOS capacity is available; the current consolidated run reached the UI suite and exposed a real accessibility-query failure that must be corrected and re-run.
...
## Exact next action
Commit the role-agnostic `todayScreen` UI-test query correction, then rerun consolidated authoritative macOS CI. If it passes, reconcile M2.9–M3.9 and the CI portions of M4.1–M4.8 in order; otherwise fix the reported real failure before starting dependent implementation.
