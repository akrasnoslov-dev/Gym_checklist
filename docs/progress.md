...
## Exact next action
Run focused `ui` against the grouped accessibility correction. Run `full` only after that UI layer passes; if it passes, reconcile M2.9–M3.9 and the CI portions of M4.1–M4.8 in order. Otherwise batch the reported same-layer failures before rerunning that layer.
