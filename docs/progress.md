...
## Exact next action
Commit the CI scope controls, run `build` against the current query correction, then run focused `ui` if compilation passes. Run `full` only after that UI layer passes; if it passes, reconcile M2.9–M3.9 and the CI portions of M4.1–M4.8 in order. Otherwise batch the reported same-layer failures before rerunning that layer.
